"""
Ridge岭回归模型：预测autoencoder_value
...
# ==================== 3. 网格搜索调参 ====================
print("\n[4] 网格搜索最优超参数...")
print("（Ridge回归训练速度很快，请稍候...）")

# Ridge回归的主要超参数是alpha（正则化强度）
# 使用对数空间搜索alpha值
param_grid = {
...
for train_idx, val_idx in kfold.split(X_train):
    ...
    model_cv = Pipeline(steps=[
        ...
        ("ridge", Ridge(
            ...
            solver=search.best_params_["ridge__solver"],
            random_state=42
        ))
    ])
...
# 创建图表目录
import os
os.makedirs("ridge_results", exist_ok=True)
...
plt.tight_layout()
plt.savefig('ridge_results/1_prediction_scatter.png', dpi=300, bbox_inches='tight')
print("  ✓ 保存: ridge_results/1_prediction_scatter.png")
plt.close()
...
plt.tight_layout()
plt.savefig('ridge_results/2_residuals.png', dpi=300, bbox_inches='tight')
print("  ✓ 保存: ridge_results/2_residuals.png")
plt.close()
...
plt.tight_layout()
plt.savefig('ridge_results/3_distribution_comparison.png', dpi=300, bbox_inches='tight')
print("  ✓ 保存: ridge_results/3_distribution_comparison.png")
plt.close()
...
plt.tight_layout()
plt.savefig('ridge_results/4_metrics_comparison.png', dpi=300, bbox_inches='tight')
print("  ✓ 保存: ridge_results/4_metrics_comparison.png")
plt.close()
...
colors = ['red' if coef < 0 else 'blue' for coef in ridge_model.coef_[indices]]
bars = ax.barh(range(len(feature_cols)), ridge_model.coef_[indices], color=colors, alpha=0.7, edgecolor='black')
...
# 添加数值标签
for i, (bar, coef) in enumerate(zip(bars, ridge_model.coef_[indices])):
    width = bar.get_width()
...
plt.tight_layout()
plt.savefig('ridge_results/5_feature_importance.png', dpi=300, bbox_inches='tight')
print("  ✓ 保存: ridge_results/5_feature_importance.png")
plt.close()
...
plt.tight_layout()
plt.savefig('ridge_results/6_error_distribution.png', dpi=300, bbox_inches='tight')
print("  ✓ 保存: ridge_results/6_error_distribution.png")
plt.close()
...
for alpha in alphas:
    ...
    train_pred = ridge_temp.predict(X_train_scaled)
    test_pred = ridge_temp.predict(X_test_scaled)
    
    train_scores.append(r2_score(y_train, train_pred))
    val_scores.append(r2_score(y_test, test_pred))

fig, ax = plt.subplots(figsize=(10, 6))
ax.semilogx(alphas, train_scores, label='Training R²', linewidth=2, marker='o', markersize=4)
ax.semilogx(alphas, val_scores, label='Test R²', linewidth=2, marker='s', markersize=4)
ax.axvline(x=search.best_params_["ridge__alpha"], color='r', linestyle='--', 
           label=f'Best α = {search.best_params_["ridge__alpha"]}', linewidth=2)
...
plt.tight_layout()
plt.savefig('ridge_results/7_regularization_path.png', dpi=300, bbox_inches='tight')
print("  ✓ 保存: ridge_results/7_regularization_path.png")
plt.close()
...
# 保存结果
df_train_result.to_csv('ridge_results/train_predictions.csv', index=False)
df_test_result.to_csv('ridge_results/test_predictions.csv', index=False)
print("  ✓ 保存: ridge_results/train_predictions.csv")
print("  ✓ 保存: ridge_results/test_predictions.csv")
...
正则化系数 (alpha): {search.best_params_['ridge__alpha']}
求解器 (solver): {search.best_params_['ridge__solver']}
...
线性假设检查:
  Ridge回归假设特征与目标变量之间存在线性关系
  如果R²较低，可能需要考虑非线性模型（如多项式特征、MLP等）

八、输出文件
-----------
预测结果:
  - ridge_results/train_predictions.csv (训练集预测结果)
  - ridge_results/test_predictions.csv (测试集预测结果)

可视化图表:
  - ridge_results/1_prediction_scatter.png (预测值散点图)
  - ridge_results/2_residuals.png (残差图)
  - ridge_results/3_distribution_comparison.png (分布对比图)
  - ridge_results/4_metrics_comparison.png (指标对比图)
  - ridge_results/5_feature_importance.png (特征重要性图)
  - ridge_results/6_error_distribution.png (误差分布图)
  - ridge_results/7_regularization_path.png (正则化路径图)
...
with open('ridge_results/report.txt', 'w', encoding='utf-8') as f:
    f.write(report)

print(report)
print("\n  ✓ 保存: ridge_results/report.txt")

print("\n" + "=" * 60)
print("所有任务完成！")
print("=" * 60)
print(f"\n所有结果已保存到 ridge_results/ 目录")
print(f"共生成 {len([f for f in os.listdir('ridge_results') if f.endswith('.png')])} 张图表和详细报告")
