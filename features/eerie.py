...
def _compute_feedback_vectorized(guess_arr, answers_arr):
    """
    向量化计算单个 guess 对所有 answers 的反馈编码（近似 Wordle 规则）。
    ...
    注意：这里没有严格处理多重字母计数，但对 (guess, answer) 的反馈是
    确定的，且候选更新和熵计算都基于同一编码，因此不会误删真正的答案，
    只会在极少数含重复字母的单词上与真实 Wordle 有轻微偏差。
    """
    n = answers_arr.shape[0]
    feedback = np.zeros(n, dtype=np.int32)

    for pos in range(5):
        g = guess_arr[pos]            # 当前位猜测字母
        col = answers_arr[:, pos]     # 所有答案该位的字母
        ...
        # yellow：在答案中但当前位不是 green
        is_yellow = is_in_answer & ~is_green

        # 当前位置的反馈编码
        pos_feedback = np.zeros(n, dtype=np.int32)
        pos_feedback[is_yellow] = 1
        pos_feedback[is_green] = 2
...
def optimal_rule_entropy_fast(answer_idx, word2idx, feedback_table):
    ...
    for try_num in range(1, 8):
        ...
        # 根据反馈更新候选掩码（使用统一的更新函数）
        candidate_mask = _update_candidate_mask(
            candidate_mask,
            guess_idx,
            answer_idx,
            words_arr=None,            # 目前更新逻辑只依赖 feedback_table，占位以兼容函数签名
...
def run_entropy_fast_for_word_direct(word, feedback_table, word2idx):
    """
    直接传递参数的版本，不使用全局变量。
    """
    ...
    answer_idx = word2idx[word]
    res = []
    # 由于熵策略确定性很强（除了第一步随机），减少重复次数
    from tqdm import tqdm
    for _ in tqdm(range(1000)):
        res.append(optimal_rule_entropy_fast(answer_idx, word2idx, feedback_table))
    return word, res
...
if __name__ == '__main__':
    ...
    # 读取答案词列表
    df = pd.read_csv('data/mcm_processed_data.csv', low_memory=False)[:1]
    words = df['word'].tolist()
    if ('manly' not in words):
        print("不在词表中")
    ...
    for word in tqdm(valid_words, total=total):
        # 使用熵优先策略（预计算反馈表版本）
        results.append(run_entropy_fast_for_word_direct(word, feedback_table, word2idx))
    ...
    # out_df['mean_simulate_random'] = out_df['results'].apply(lambda x: sum(x)/len(x))
    # out_df['1_try_simulate_random'] = out_df['results'].apply(lambda xs: sum(1 for v in xs if v == 1) / len(xs) if xs else 0)
    # out_df['2_try_simulate_random'] = out_df['results'].apply(lambda xs: sum(1 for v in xs if v == 2) / len(xs) if xs else 0)
    # out_df['3_try_simulate_random'] = out_df['results'].apply(lambda xs: sum(1 for v in xs if v == 3) / len(xs) if xs else 0)
    # out_df['4_try_simulate_random'] = out_df['results'].apply(lambda xs: sum(1 for v in xs if v == 4) / len(xs) if xs else 0)
    # out_df['5_try_simulate_random'] = out_df['results'].apply(lambda xs: sum(1 for v in xs if v == 5) / len(xs) if xs else 0)
    # out_df['6_try_simulate_random'] = out_df['results'].apply(lambda xs: sum(1 for v in xs if v == 6) / len(xs) if xs else 0)
    # out_df['7_try_simulate_random'] = out_df['results'].apply(lambda xs: sum(1 for v in xs if v == 7) / len(xs) if xs else 0)
    
    
    # out_df['mean_simulate_freq'] = out_df['results'].apply(lambda x: sum(x)/len(x))
    # out_df['1_try_simulate_freq'] = out_df['results'].apply(lambda xs: sum(1 for v in xs if v == 1) / len(xs) if xs else 0)
    # out_df['2_try_simulate_freq'] = out_df['results'].apply(lambda xs: sum(1 for v in xs if v == 2) / len(xs) if xs else 0)
    # out_df['3_try_simulate_freq'] = out_df['results'].apply(lambda xs: sum(1 for v in xs if v == 3) / len(xs) if xs else 0)
    # out_df['4_try_simulate_freq'] = out_df['results'].apply(lambda xs: sum(1 for v in xs if v == 4) / len(xs) if xs else 0)
    # out_df['5_try_simulate_freq'] = out_df['results'].apply(lambda xs: sum(1 for v in xs if v == 5) / len(xs) if xs else 0)
    # out_df['6_try_simulate_freq'] = out_df['results'].apply(lambda xs: sum(1 for v in xs if v == 6) / len(xs) if xs else 0)
    # out_df['7_try_simulate_freq'] = out_df['results'].apply(lambda xs: sum(1 for v in xs if v == 7) / len(xs) if xs else 0)
    ...
    # df = df.merge(out_df[['word', 'mean_simulate_random', '1_try_simulate_random',
    #                       '2_try_simulate_random', '3_try_simulate_random', '4_try_simulate_random',
    #                       '5_try_simulate_random', '6_try_simulate_random', '7_try_simulate_random']], on='word', how='left')
    # df = df.merge(out_df[['word', 'mean_simulate_freq', '1_try_simulate_freq',
    #                     '2_try_simulate_freq', '3_try_simulate_freq', '4_try_simulate_freq',
    #                     '5_try_simulate_freq', '6_try_simulate_freq', '7_try_simulate_freq']], on='word', how='left')
    # df.to_csv("./data/eerie.csv", index=False)
    out_df.to_csv("sum_manly_results_entropy.csv", index=False)
    print("已完成")
